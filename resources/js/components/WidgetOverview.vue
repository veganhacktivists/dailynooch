<template>
  <div class="container">
    <div v-if="state === 'error'" class="alert alert-danger" role="alert">
      <i class="fas fa-kiwi-bird"></i>&nbsp; There was a problem loading the
    ...
    async mounted() {
      try {
        ...
        ;[
          ...
          let targetContainer = mapping.targetContainer,
            widgetOrder = mapping.widgetOrder
          widgetOrder
            .filter((widgetType) => widgetsMap.has(widgetType))
            .forEach((widgetType) =>
              targetContainer.push(widgetsMap.get(widgetType)),
            )
        })
      } catch (err) {
        this.state = 'error'
